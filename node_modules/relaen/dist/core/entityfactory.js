...
class EntityFactory {
     ...
     * @param schema -        数据库名
     */
    static addEntityConfig(entity, tblName, schema) {
        const entityName = entity.name;
        if (EntityFactory.entityConfigs.has(entityName)) {
            const cfg = EntityFactory.entityConfigs.get(entityName);
            cfg.setEntityClass(entity);
            cfg.setTableName(tblName);
            cfg.setSchemaName(schema);
        }
        else {
            EntityFactory.entityConfigs.set(entityName, new entityconfig_1.EntityConfig({
                entity: entity,
                table: tblName,
                schema: schema
            }));
    ...
    static addPKey(entityName, propName, cfg) {
        ...
        if (cfg.generator === 'table' && !cfg.keyName || cfg.generator === 'sequence' && !cfg.seqName) {
            throw new error_1.RelaenError("0050", entityName);
        }
        const ecfg = this.entityConfigs.get(entityName);
        ecfg.setId(cfg);
    }
    /**
     * 设置版本号字段名
     ...
     */
    static addVersion(entityName, propName) {
        this.checkAndNew(entityName);
        const ecfg = this.entityConfigs.get(entityName);
     ...
     * @param entityName -    实体类名
     */
    static checkAndNew(entityName) {
        if (!this.entityConfigs.has(entityName)) {
            this.entityConfigs.set(entityName, new entityconfig_1.EntityConfig());
        }
    }
    /**
     ...
     * @returns             实体配置
     */
    static getEntityConfig(entityName) {
     ...
     * @returns             是否存在
     */
    static hasEntityConfig(entityName) {
            ...
            for (const dirent of dir) {
                if (dirent.isDirectory()) {
                    ...
                    if (reg.test(dirent.name)) {
                        await Promise.resolve(`${pathMdl.resolve(dirPath, dirent.name)}`).then(s => require(s));
                    }
                }
            }
        }
    ...
    static setIdValue(entity, value) {
        const cfg = this.getEntityConfig(entity.constructor.name);
        if (cfg) {
 ...
 * 实体类集
 */
EntityFactory.entityConfigs = new Map();
