...
class Translator {
    ...
    constructor(entityName) {
        ...
        this.mainEntityCfg = entityfactory_1.EntityFactory.getEntityConfig(this.mainEntityName);
        if (!this.mainEntityCfg) {
            throw new error_1.RelaenError("0010", this.mainEntityName);
        }
    }
    /**
     * entity转insert sql
    ...
    entityToInsert(entity) {
        ...
        const qArr = [];
        //id字段名
        const idField = this.mainEntityCfg.columns.get(this.mainEntityCfg.id.name).name;
        for (const [key, cfg] of this.mainEntityCfg.columns) {
            ...
            if (cfg.refName) { //外键，只取主键
                ...
                v = refEn && refEn instanceof baseentity_1.BaseEntity ? entityfactory_1.EntityFactory.getIdValue(refEn) : null;
            }
            else {
                v = entity[key];
            }
        ...
        arr.push('(' + qArr.join(',') + ')');
        //针对不同的数据库，可能存在附加串
        const extra = connectionmanager_1.ConnectionManager.provider.insertReturn(idField);
        if (extra) {
            arr.push(extra);
        }
    ...
    entityToUpdate(entity, ignoreUndefinedValue) {
        ...
        for (const [key, cfg] of this.mainEntityCfg.columns) {
            ...
            if (key === this.mainEntityCfg.version) {
                if (v === undefined || v === null) {
                    throw new error_1.RelaenError('0408', this.mainEntityName);
                }
                if (typeof v !== 'number') {
                    throw new error_1.RelaenError('0409', this.mainEntityName);
                }
                versionValue = v++;
                versionField = fn;
            }
        ...
        if (!idValue) {
            throw new error_1.RelaenError('0021', this.mainEntityCfg.id.name);
        }
        arr.push(fields.join(','));
        //where
        arr.push('WHERE');
    ...
    toDelete(entity) {
        ...
        if (entity instanceof baseentity_1.BaseEntity) {
            eo = entityfactory_1.EntityFactory.getEntityConfig(entity.constructor.name);
            idValue = entityfactory_1.EntityFactory.getIdValue(entity);
        }
        else {
            idValue = entity;
            eo = this.mainEntityCfg;
        }
        const idName = eo.getIdName();
        if (!idName || !idValue) {
            throw new error_1.RelaenError("0025");
        }
    ...
    handleSelectFields(arr, entityName) {
        ...
        for (let i = 0; i < arr.length; i++) {
            let fn = arr[i].trim();
            const ind1 = fn.indexOf('(');
            if (ind1 !== -1) { //函数内
                const foo = fn.substring(0, ind1).toLowerCase();
     ...
     */
    handleOneField(field, entityName, linkName, isCond, asField) {
        field = field.trim();
        if (field === '') {
            return '';
        }
        ...
        if (field === '*') {
            ...
            if (entityName && entityfactory_1.EntityFactory.hasEntityConfig(entityName)) {
                const orm = entityfactory_1.EntityFactory.getEntityConfig(entityName);
                const arr = [];
        ...
        let index = 0;
        //先判断是否为主实体
        if (entityfactory_1.EntityFactory.hasEntityConfig(arr[0])) {
            entityName = arr[0];
            index++;
        }
        entityName = entityName || this.mainEntityName;
        ...
        const orm = entityfactory_1.EntityFactory.getEntityConfig(entityName);
        if (!orm) {
            throw new error_1.RelaenError('0010', entityName);
        }
        for (let i = index; i < arr.length; i++) {
            ...
            if (!orm.columns.has(arr[i])) {
                throw new error_1.RelaenError('0022', entityName, arr[i]);
            }
            const co = orm.columns.get(arr[i]);
            //外键对象，则直接切换为外键对象
            if (co.refName) {
                const rel = orm.getRelation(arr[i]);
                ...
                if (i === arr.length - 1) {
                    if (isCond) { //如果为条件，则转换为主键
                        const eo = entityfactory_1.EntityFactory.getEntityConfig(rel.entity);
    ...
    handleCondition(params) {
        ...
        Object.getOwnPropertyNames(params).forEach((item, ii) => {
            ...
            if (v instanceof baseentity_1.BaseEntity) {
                v = entityfactory_1.EntityFactory.getIdValue(v);
            }
            //前置字符串，通常为'('
            if (vobj && vobj.before) {
            ...
            if (rel === 'IN' || rel === 'NOT IN') {
                if (!Array.isArray(v)) {
                    throw new error_1.RelaenError('0404');
                }
            ...
            if (rel === 'BETWEEN' || rel === 'NOT BETWEEN') {
                if (!Array.isArray(v) || v.length !== 2) {
                    throw new error_1.RelaenError('0405');
                }
                placeholder = ' ? AND ?';
            }
            //字段和值
    ...
    getSelectSql() {
        ...
        // 处理mssql的锁机制
        if ((this.lockMode === types_1.ELockMode.PESSIMISTIC_READ || this.lockMode === types_1.ELockMode.PESSIMISTIC_WRITE) && relaenmanager_1.RelaenManager.dialect === 'mssql') {
            arr.push(connectionmanager_1.ConnectionManager.provider.lock(this.lockMode === types_1.ELockMode.PESSIMISTIC_READ ? types_1.ELockType.ROWREAD : types_1.ELockType.ROWWRITE));
        }
        //处理left join
        const entities = [];
        const joins = [];
        for (const [key, link] of this.linkNameMap) {
            ...
            const orm = entityfactory_1.EntityFactory.getEntityConfig(link['entity']);
            const al1 = link['alias'];
            const al2 = this.linkNameMap.get(link['from'])['alias'];
            const co = link['co'];
        ...
        if (this.fromTables) {
            for (const t of this.fromTables) {
                ...
                const orm = entityfactory_1.EntityFactory.getEntityConfig(t);
                //添加inner join
    ...
    getDeleteSql(notNeedAlias) {
        if (!this.whereObject && !relaenmanager_1.RelaenManager.fullTableOperation) {
            throw new error_1.RelaenError('0406');
        }
        const arr = [];
