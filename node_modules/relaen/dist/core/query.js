...
class Query {
    ...
    constructor(em, entityClassName) {
        if (entityClassName && !entityfactory_1.EntityFactory.getEntityConfig(entityClassName)) {
            throw new error_1.RelaenError("0020", entityClassName);
        }
    ...
    setParameters(valueArr) {
        valueArr.forEach((value, i) => {
            //对于entity，只获取其主键
            const v = value instanceof baseentity_1.BaseEntity ? entityfactory_1.EntityFactory.getIdValue(value) : value;
    ...
    async getResultList(start, limit, notEntity) {
        ...
        this.setStart(start);
        this.setLimit(limit);
        const results = await sqlexecutor_1.SqlExecutor.exec(this.entityManager, this.execSql, this.paramArr, this.start, this.limit);
     ...
     */
    genEntity(r) {
        const ecfg = entityfactory_1.EntityFactory.getEntityConfig(this.entityClassName);
        const entity = new ecfg.entity();
        //存储外键值
        Object.getOwnPropertyNames(r).forEach((field) => {
            ...
            const ind = field.indexOf('_');
            //别名
            const alias = field.substring(0, ind);
            const propName = field.substring(ind + 1);
            //主表
        ...
        function handleSubEntity(entity, arr, key, value) {
            let eo = entity[arr[0]];
            if (!eo) {
                let orm = entityfactory_1.EntityFactory.getEntityConfig(entity.constructor.name);
                //获取关系对象
                const rel = orm.getRelation(arr[0]);
                if (!rel) {
                    return;
                }
                orm = entityfactory_1.EntityFactory.getEntityConfig(rel.entity);
                if (!orm) {
                    return;
                }
                eo = new orm.entity();
