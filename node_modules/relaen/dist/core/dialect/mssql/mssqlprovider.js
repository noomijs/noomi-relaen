...
class MssqlProvider extends baseprovider_1.BaseProvider {
    ...
    async exec(connection, sql, params) {
        ...
        const tx = transactionfactory_1.TransactionFactory.get();
        if (tx) {
    ...
    handleStartAndLimit(sql, start, limit) {
        ...
        function handleOrder() {
            if (!/order\s+by/i.test(sql)) {
                ...
                let orderBy;
                const cfg = entityfactory_1.EntityFactory.getEntityCfgByTblName(tbl);
                if (cfg) {
     ...
     * @returns         sequence 值
     */
    async getSequenceValue(em, seqName, schema) {
        const query = em.createNativeQuery("SELECT NEXT VALUE FOR " + seqName);
